...
uint8_t I2C_WriteDeviceRegister(uint8_t RegisterAddr, uint8_t RegisterValue)
{
  ...
  /**
    * @brief  Writes a value in a register of the PCF device through I2C.
    */
uint8_t I2C_WritePCFRegister(uint8_t DeviceAddr, uint8_t RegisterValue)
{
    /* Begin the configuration sequence */
    I2C_GenerateSTART(IOE_I2C, ENABLE);
    ...
    /* Success return (timeout returns 0) */
    return 1;
}
...
/**
  * @brief  Reads a register of the PCF8574 device through I2C without DMA.
  */

uint8_t I2C_ReadPCFRegister(uint8_t DeviceAddr)
{
  uint8_t tmp = 0;
